class IncomeHandle:
    ...
    def __init__(self, gross_salary, notionals=None):
        ...
        self._notionals = notionals or 0
    ...
    def get_nic(self):
        ...
        l_bracket = min(thres_u['figure'] - thres_l['figure'], self._gross - thres_l['figure']) * thres_l.get('rate')
    ...
    def get_tax(self):
        ...
        thresholds = {'allowance': 12570, 'basic': 50270, 'higher': 150000, 'additional': 150000, 'extra': 0}
        
        if salary >= 100000:
            thresholds['extra'] = thresholds['allowance'] - max(0, thresholds['allowance'] - (salary - 100000) / 2)

        basic = max(0, min(salary - thresholds['allowance'], thresholds['basic'] - thresholds['allowance']))
        higher = max(0, min(salary - thresholds['basic'], thresholds['higher'] - thresholds['basic']))
        additional = max(0, salary - thresholds['additional'])
 
        return round((basic * 0.2 + (higher + thresholds['extra']) * 0.4 + additional * 0.45)  / 12, 0)
